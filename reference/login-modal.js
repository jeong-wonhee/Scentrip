/* ============================================================
   ...
   쓰는 법: ScentLogin.require({ reason: "save", then: () => { ...저장... } })
     reason — save | like | helpful | review | route (모달 설명 문구)
...
const ScentLogin = (() => {
  const REASON = {
    save:    "마음에 드는 곳을 저장하려면 로그인이 필요해요.",
    like:    "동선을 찜하려면 로그인이 필요해요.",
    helpful: "도움돼요를 남기려면 로그인이 필요해요.",
    review:  "리뷰를 쓰려면 로그인이 필요해요.",
    route:   "이 동선을 내 여행으로 가져오려면 로그인이 필요해요.",
  };
  const DONE = {
    ...
    scrim.innerHTML = `
      <div class="modal-box lm-box" role="dialog" aria-modal="true" aria-labelledby="lmTitle" aria-describedby="lmDesc">
        <div class="modal-head">
          <div>
            <h2 id="lmTitle">로그인하고 계속할까요?</h2>
            <p id="lmDesc"></p>
          </div>
          <button class="modal-close" type="button" data-lm-close aria-label="닫기">${X}</button>
        </div>
        ...
        <p class="lm-note">처음이라면 간단한 가입 화면으로 이어지고, 가입을 마치면 이 화면으로 돌아와요.</p>
        <button class="lm-later" type="button" data-lm-close>나중에 할게요</button>
        <!-- 검수용 — 개발 전달 시 제거 (실서비스는 Google 콜백 결과로 분기) -->
        <label class="lm-qa">검수용 · Google 인증 결과
          <select id="lmQa">
  ...
  function open(opts) {
    ...
    $("lmDesc").textContent = REASON[pending.reason] || "이 기능은 로그인 후 쓸 수 있어요.";
    $("lmAlert").hidden = true;
    setBusy(false);
    scrim.hidden = false;
